...
public class CrownCourtEligibilityService implements EligibilityChecker {
    ...
    public boolean isEligibilityCheckRequired(MeansAssessmentRequestDTO assessmentRequest) {
        ...
        if (isEitherWayAndCommittedForTrial) {
            ...
            if (!isFirstMeansAssessment) {
                boolean isInitResultPass = InitAssessmentResult.PASS.equals(
                        InitAssessmentResult.getFrom(initialAssessment.getInitResult()));
                LocalDateTime magsOutcomeDate = Objects.requireNonNullElse(
                        repOrder.getMagsOutcomeDateSet(),
                        DateUtil.stringToLocalDateTime(repOrder.getMagsOutcomeDate(), DateUtil.DATE_FORMAT));
                boolean isDateCreatedAfterMagsOutcome =
                        initialAssessment.getDateCreated().isAfter(magsOutcomeDate);
        ...
        return previousAssessments.noneMatch(this::hasDisqualifyingResult);
    }
    ...
    private Assessment getLatestAssessment(RepOrderDTO repOrder, Integer financialAssessmentId) {
        return getPreviousAssessments(repOrder)
                ...
                .max(comparing(Assessment::getDateCreated))
                .orElse(null);
    }
