...
public class FullMeansAssessmentService implements AssessmentService {
    ...
    public MeansAssessmentDTO execute(
        ...
        BigDecimal totalDisposableIncome =
                getDisposableIncome(totalAggregatedIncome, expenditureTotal, adjustedLivingAllowance);
        ...
        return MeansAssessmentDTO.builder()
                ...
                .fullAssessmentResult(
                        status.equals(CurrentStatus.COMPLETE)
                                ? getResult(totalDisposableIncome, requestDTO, assessmentCriteria)
                                : null)
                .adjustedLivingAllowance(adjustedLivingAllowance)
                .totalAggregatedExpense(getAnnualAggregatedExpenditure(totalAggregatedIncome, totalDisposableIncome))
    ...
    BigDecimal getDisposableIncome(
            BigDecimal totalAggregatedIncome, BigDecimal expenditureTotal, BigDecimal adjustedLivingAllowance) {
        return setStandardScale(totalAggregatedIncome.subtract(expenditureTotal.add(adjustedLivingAllowance)));
    }

    BigDecimal getAdjustedLivingAllowance(
            MeansAssessmentRequestDTO requestDTO, AssessmentCriteriaEntity assessmentCriteria) {
    ...
    FullAssessmentResult getResult(
