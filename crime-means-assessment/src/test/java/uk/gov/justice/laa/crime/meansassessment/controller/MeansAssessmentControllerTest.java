...
import static uk.gov.justice.laa.crime.meansassessment.data.builder.TestModelDataBuilder.ASSESSMENT_DATE;
import static uk.gov.justice.laa.crime.meansassessment.data.builder.TestModelDataBuilder.FULL_THRESHOLD;
...
class MeansAssessmentControllerTest {
    ...
    private static final String FA_THRESHOLD_ENDPOINT_URL =
            "/api/internal/v1/assessment/means/fullAssessmentThreshold/";
    ...
    void createAssessment_success() throws Exception {
        var initialMeansAssessmentRequest = TestModelDataBuilder.getApiCreateMeansAssessmentRequest(IS_VALID);
        var initialMeansAssessmentRequestJson = objectMapper.writeValueAsString(initialMeansAssessmentRequest);
        var initialMeansAssessmentResponse = TestModelDataBuilder.getInitMeansAssessmentResponse(IS_VALID);
    ...
    void updateAssessment_success() throws Exception {
        var updateAssessmentRequest = TestModelDataBuilder.getApiUpdateMeansAssessmentRequest(IS_VALID);
        var updateAssessmentRequestJson = objectMapper.writeValueAsString(updateAssessmentRequest);
        var updateAssessmentResponse = TestModelDataBuilder.getFullMeansAssessmentResponse(IS_VALID);
    ...
    void createAssessment_RequestObjectFailsValidation() throws Exception {
        var createAssessmentRequest = TestModelDataBuilder.getApiCreateMeansAssessmentRequest(!IS_VALID);
    ...
    void createAssessment_ServerError_RequestBodyIsMissing() throws Exception {
        mvc.perform(buildRequestGivenContent(HttpMethod.POST, "", ENDPOINT_URL)).andExpect(status().is4xxClientError());
    }
    ...
    void updateAssessment_ServerError_RequestBodyIsMissing() throws Exception {
        mvc.perform(buildRequestGivenContent(HttpMethod.PUT, "", ENDPOINT_URL)).andExpect(status().is4xxClientError());
    }
    ...
    void givenValidParam_whenGetOldAssessmentInvoked_shouldSuccess() throws Exception {
        ...
        mvc.perform(buildRequestGivenContent(HttpMethod.GET, "", ENDPOINT_URL + "/" + MEANS_ASSESSMENT_ID, true))
                .andExpect(status().isOk());
    }
    ...
    void givenValidParam_whenFullAssessmentThresholdInvoked_shouldSuccess() throws Exception {
        ...
        mvc.perform(buildRequestGivenContent(
                        HttpMethod.GET, "", FA_THRESHOLD_ENDPOINT_URL + "/" + ASSESSMENT_DATE, true))
                .andExpect(status().isOk())
    ...
    void givenValidParam_whenRollbackInvoked_shouldSuccess() throws Exception {
        mvc.perform(buildRequestGivenContent(
                        HttpMethod.PATCH, "", ENDPOINT_URL + "/rollback/" + MEANS_ASSESSMENT_ID, true))
                .andExpect(status().isOk());
    }
}
